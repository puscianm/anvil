...
def adaround_layer(layer, inputs, num_iterations, beta_range, reg_param, bitwidth, per_channel):
    ...
    scale_in, zp_in = get_qparams(inputs, qmin, qmax, per_channel=per_channel)
    ...
    final_q_adarounded = torch.clamp(torch.floor(weight / scale_w + zp_w) + h_alpha, qmin, qmax)
    final_w_q = scale_w * (final_q_clamped - zp_w)
...
class AdaRoundModelWrapper:
    ...
    def apply_adaround_to_conv_layers(self, bitwidth=8, reg_param = 0.07, per_channel = False, num_iterations=3500, beta_range=(40, 2)):
        layers_h_alphas = []
